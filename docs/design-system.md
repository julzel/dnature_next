...
- Search metadata is generated by `features/Faq/seo.js` from that same visible
  content. It publishes an absolute-URL `FAQPage` and `BreadcrumbList`, Spanish
  language metadata, stable question anchors, update date, publisher, and only
  citations that customers can also open in the answer.
