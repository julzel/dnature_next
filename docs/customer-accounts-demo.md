...
The demo shows how a DNAture account can add value beyond the food products themselves: it helps a client organize pet profiles, understand approximate food needs, prepare recurring purchases, and receive relevant suggestions in one simple place.
...
- Shortcuts to pets, saved carts, and profile settings.
...
A concise presentation takes about five minutes:
...
5. Open “Mis carritos,” choose “Alimento de la quincena,” and use it to populate the existing site cart.
6. Open “Mi perfil” to show address reuse, personalization controls, sign-out behavior, and demo reset.
7. Return to the access page and demonstrate the email flow using code `123456` if time allows.
...
- Google, Facebook, email verification, recommendations, and reminders do not call external services.
...
- Unit coverage for the versioned state, sample data, pet lifecycle, sign-in merging, cart limits, and invalid stored data.
- Browser tests for the full stakeholder sample, email-code flow, pet creation, and saved-cart restoration.
- Automated accessibility checks for the sign-in page and populated dashboard.
...
### 6. Personalization and customer value
...
### 8. Security, privacy, and legal launch gates
...
7. Add transparent rule-based recommendations.
8. Complete legal copy, deletion/export, security review, observability, and support tools.
9. Run a staff-only pilot, then a limited customer beta behind a feature flag.
10. Review evidence and approve general availability.
